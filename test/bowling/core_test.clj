...
(deftest test-build-frames
  ...
  (testing "with one roll"
    (is (= (list (struct Frame 1 0 1)) (build-frames '(1)))))
  (testing "with two rolls"
    (is (= (list (struct Frame 1 2 3)) (build-frames '(1 2)))))
  (testing "with three rolls"
    (is (= (list (struct Frame 1 2 3) (struct Frame 5 0 5))
           (build-frames '(1 2 5)))))
  (testing "with spare, one after"
    (is (= (list (struct Frame 3 7 12) (struct Frame 2 0 2))
           (build-frames '(3 7 2)))))
  (testing "with spare, two after"
    (is (= (list (struct Frame 3 7 12) (struct Frame 2 4 6))
           (build-frames '(3 7 2 4)))))
  (testing "with strike, one after"
    (is (= (list (struct Frame 10 0 12) (struct Frame 2 0 2))
           (build-frames '(10 2)))))
  (testing "with strike, two after"
    (is (= (list (struct Frame 10 0 16) (struct Frame 2 4 6))
           (build-frames '(10 2 4)))))
  (testing "with strike, spare, one after"
    (is (= (list (struct Frame 10 0 20)
                 (struct Frame 3 7 14)
                 (struct Frame 4 0 4))
           (build-frames '(10 3 7 4)))))
  (testing "with strike, strike, one after"
    (is (= (list (struct Frame 10 0 24)
                 (struct Frame 10 0 14)
                 (struct Frame 4 0 4))
           (build-frames '(10 10 4)))))
  (testing "with strike, strike, strike"
    (is (= (list (struct Frame 10 0 30)
                 (struct Frame 10 0 20)
                 (struct Frame 10 0 10))
